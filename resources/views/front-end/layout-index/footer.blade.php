		<!-- footer_section - start
		================================================== -->
		<footer class="footer_section clearfix">
			<div class="footer_widget_area sec_ptb_100 clearfix">
				<div class="container">
					<div class="row justify-content-lg-between">
						<div class="col-lg-4 col-md-4 col-sm-12 col-sm-12">
							<div class="footer_about" data-aos="fade-up" data-aos-delay="100">
								<div class="brand_logo mb_30">
									<a href="index.html">
										<img style="max-width: 100%; max-height: 120px;" src="front_end/assets/images/logo-01.png" srcset="front_end/assets/images/logo-01.png 2x" alt="logo_not_found">
									</a>
								</div>
								<p class="mb_15">
								This text is an example of a text that can be replaced in the same space. This text was generated from the Arabic text generator,
								</p>
								<p class="mb_15">
								This text is an example of a text that can be replaced in the same space. This text was generated from the Arabic text generator,
								</p>

							</div>
						</div>

						<div class="col-lg-3 col-md-4 col-sm-12 col-sm-12">
							<div class="footer_contact_info" data-aos="fade-up" data-aos-delay="200">
								<h3 class="footer_widget_title">Contact Us:</h3>
								<ul class="ul_li_block clearfix">
									<li>
										<strong><i class="fas fa-map-marker-alt"></i> Main Office Address:</strong>
										<p class="mb-0">
											Unit 9, Manor Industrial Estate, Lower Wash Lane, Warrington, WA4
										</p>
									</li>
									<li><i class="fas fa-clock"></i> 8:00am-9:30pm</li>
									<li><i class="far fa-angle-right"></i> Other Office Locations</li>
									<li><i class="fas fa-envelope"></i> <strong>rotorseml@eml.fr</strong></li>
									<li><i class="fas fa-phone"></i> <strong>+880 1680 6361 89</strong></li>
								</ul>
							</div>
						</div>

						<div class="col-lg-4 col-md-4 col-sm-12 col-sm-12">
							<div class="footer_useful_links" data-aos="fade-up" data-aos-delay="300">
								<h3 class="footer_widget_title">Information:</h3>
								<ul class="ul_li_block clearfix">
									<li><a href="/gallery"><i class="fal fa-angle-right"></i>our Cars</a></li>
									<li><a href="/about"><i class="fal fa-angle-right"></i> About Us</a></li>
									<li><a href="/contactUs"><i class="fal fa-angle-right"></i> Contact Us</a></li>
									<li><a href="/privacy"><i class="fal fa-angle-right"></i> Privacy Police</a></li>
								</ul>
							</div>
						</div>
					</div>
				</div>
			</div>

			<div class="footer_bottom text-white clearfix" data-bg-color="#000C21">
				<div class="container">
					<div class="row align-items-center justify-content-lg-between">
						<div class="col-lg-6 col-md-6 col-sm-12 col-xs-12">
							<p class="copyright_text mb-0">Copyright © 2020. Rotors by <a class="author_links text-white" href="#!">Merkulove</a></p>
						</div>

						<div class="col-lg-6 col-md-6 col-sm-12 col-xs-12">
							<ul class="primary_social_links ul_li_right clearfix">
								<li><a href="#!"><i class="fab fa-facebook-f"></i></a></li>
								<li><a href="#!"><i class="fab fa-instagram"></i></a></li>
								<li><a href="#!"><i class="fab fa-twitter"></i></a></li>
								<li><a href="#!"><i class="fab fa-youtube"></i></a></li>
							</ul>
						</div>
					</div>
				</div>
			</div>
		</footer>
		<!-- footer_section - end
		================================================== -->

		
		<!-- fraimwork - jquery include -->
		<script src="front_end/assets/js/jquery-3.5.1.min.js"></script>
		<script src="front_end/assets/js/popper.min.js"></script>
		<script src="front_end/assets/js/bootstrap.min.js"></script>

		<!-- animation - jquery include -->
		<script src="front_end/assets/js/aos.js"></script>
		<script src="front_end/assets/js/parallaxie.js"></script>

		<!-- carousel - jquery include -->
		<script src="front_end/assets/js/slick.min.js"></script>

		<!-- popup - jquery include -->
		<script src="front_end/assets/js/magnific-popup.min.js"></script>

		<!-- select ontions - jquery include -->
		<script src="front_end/assets/js/nice-select.min.js"></script>

		<!-- isotope - jquery include -->
        <script src="front_end/assets/js/isotope.pkgd.js"></script>
        <script src="front_end/assets/js/imagesloaded.pkgd.min.js"></script>
        <script src="front_end/assets/js/masonry.pkgd.min.js"></script>

		<!-- google map - jquery include -->
		<script src="https://maps.googleapis.com/maps/api/js?key=AIzaSyDk2HrmqE4sWSei0XdKGbOMOHN3Mm2Bf-M&ver=2.1.6"></script>
        <script src="front_end/assets/js/gmaps.min.js"></script>

		<!-- pricing range - jquery include -->
		<script src="front_end/assets/js/jquery-ui.js"></script>

		<!-- counter - jquery include -->
		<script src="front_end/assets/js/waypoint.js"></script>
		<script src="front_end/assets/js/counterup.min.js"></script>

		<!-- contact form - jquery include -->
        <script src="front_end/assets/js/validate.js"></script>

		<!-- mobile menu - jquery include -->
        <script src="front_end/assets/js/mCustomScrollbar.js"></script>

		<!-- custom - jquery include -->
		<script src="front_end/assets/js/custom.js"></script>

		
	</body>
</html>